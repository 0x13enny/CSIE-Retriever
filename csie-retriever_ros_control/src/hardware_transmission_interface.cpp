  ...
  : update_freq_(update_freq)
{
  ...
  retriever_port.set_port("/dev/ttyUSB0");
  retriever_port.set_baudRate(9600);
  retriever_port.set_dataBits(8);
  retriever_port.set_stopBit(1);
  retriever_port.set_parity(false);
  retriever_port.set_hardwareFlowControl(false);

  retriever_port.writeData("1ANSW0\r");
  retriever_port.writeData("2ANSW0\r");
}
...
void HwTmIntf::update()
{
  ...
  retriever_port.writeData(ss_cmd_vel_.str());
  ...
  retriever_port.writeData(ss_curr_vel_.str());
  hw_data_.act_curr_pos_[0] = ((float)atoi((retriever_port.readData()).c_str()) / ENC_FULL) * (2 * PI);


  ss_curr_vel_.str("");
  ss_curr_vel_ << 2 << "pos\r"; // Left wheel (node 2)
  retriever_port.writeData(ss_curr_vel_.str());
  hw_data_.act_curr_pos_[1] = -((float)atoi((retriever_port.readData()).c_str()) / ENC_FULL) * (2 * PI);
